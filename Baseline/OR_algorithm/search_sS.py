...
def search_sS(env, search_range=np.arange(0.0, 12.1, 1)):
    ...
    for S1 in search_range: 
        for s1 in np.arange(0, S1 + 0.1, 1):
            for S2 in search_range: 
                for s2 in np.arange(0, S2 + 0.1, 1):
                    print(S1,s1,S2,s2)
                    reward, _       = sS_policy(env, [[S1],[S2]], [[s1],[s2]])
                    reward      = reward.sum()
                    if reward > max_reward:
                        best_S = [[S1],[S2]]
                        best_s = [[s1],[s2]]
                        max_reward = reward
                    # best_S          = np.where(reward > max_reward, [[S1],[S2]], best_S)
                    # best_s          = np.where(reward > max_reward, [[s1],[s2]], best_s)
                    # max_reward      = np.where(reward > max_reward, reward, max_reward)
    best_S = np.ones((env.warehouse_count, sku_count)) * best_S
    best_s = np.ones((env.warehouse_count, sku_count)) * best_s
    return np.ones((env.warehouse_count, sku_count)) * best_S, np.ones((env.warehouse_count, sku_count)) * best_s

def sS_hindsight(env_name, vis_path, render = True):
    ...
    # JUST FOR TEST 
    # sku_count   = len(env_train.get_sku_list())
    # best_S      = np.ones((sku_count))*3
    # best_s      = np.ones((sku_count))*3
    # Ss_path = "best_Ss/sku50.2_stores.standard"
    # with open(Ss_path, 'rb') as file:  
    #     # 从文件中加载对象  
    #     Ss_levels = pickle.load(file)
    #     best_S, best_s = Ss_levels
    print(best_S, best_s)
...
def sS_static(env_name, vis_path):
    ...
    best_S, best_s = search_sS(env_train)
    best_Ss = np.array([best_S, best_s])
    with open("best_Ss/{}","wb".format(env_name)) as f:
        pickle.dump(best_Ss,f)
    env_test = make_env(env_name, wrapper_names=["OracleWrapper"], mode='test', vis_path=vis_path)
    _, balance = sS_policy(env_test, best_S, best_s)
    env_test.render()
    return balance
...
if __name__ == "__main__":
    env_name = "sku100.2_stores.standard"
    # env_name = "similar_order_cost"
    exp_name = "search_sS"
    ...
    balance = sS_static(env_name=env_name,vis_path=vis_path)
