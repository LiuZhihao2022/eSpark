...
def run_sequential(args, logger):
    if not args.checkpoint_maskfunc_path:
        logger.console_logger.info("No exploration function, please check the path of exploration function!")
        return
    ...
    try:
        if module_name in sys.modules:  
            module = sys.modules[module_name]  # TODO:为什么函数地址变了但是执行的还是原来的那个函数？
            module = importlib.reload(module)  
        else:  
            module = importlib.import_module(module_name)
        mask_func = getattr(module, "compute_mask")
    except Exception as e:
        logger.console_logger.info(f"Checkpoint can not be loaded due to \n{str(e)}")
        logger.console_logger.info("Exiting...")
        return
    
    # Init runner so we can get env info
    # Only one runner is allowed
    runner = r_REGISTRY["parallel_load_mask"](args=args, logger=logger, mask_func=mask_func)
    ...
    while runner.t_env <= args.t_max:
        ...
        with torch.no_grad():
            # episode_batch, train_stats, _, train_old_return = runner.run(test_mode=True)
            episode_batch, train_stats, _, train_old_return = runner.run(test_mode=False)
            wandb_dict = {}
            wandb_dict.update({
