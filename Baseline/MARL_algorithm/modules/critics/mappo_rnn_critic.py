...
class MAPPORNNCritic(nn.Module):
    ...
    def save_models(self, path, postfix = ""):
        torch.save(self.state_dict(), "{}/critic".format(path)+postfix+".th")
        
    def load_models(self, path, postfix = ""):
        self.load_state_dict(torch.load("{}/critic".format(path) + postfix+".th", map_location=lambda storage, loc: storage))