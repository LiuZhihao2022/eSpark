            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if idx in envs_not_terminated:  # We produced actions for this env
                    if not terminated[
                        ...
                        parent_conn.send(("step_downstream_fixed", (cpu_actions[action_idx], self.set_stock_levels)))
...
def env_worker(remote, env_fn):
    ...
    while True:
        ...
        if cmd == "step_downstream_fixed":
            action_from_rl, replenish = data
            ...
            # only the reward of the first layer is preserved
            env_info['cur_balance'] = np.array([env_info['cur_balance'][0]])
            env_info['individual_rewards'] = np.array([env_info['individual_rewards'][0]])
