...
import pickle
from Baseline.OR_algorithm.search_sS import search_sS
from ReplenishmentEnv import make_env
# Based (very) heavily on SubprocVecEnv from OpenAI Baselines
# https://github.com/openai/baselines/blob/master/baselines/common/vec_env/subproc_vec_env.py
class ParallelRunnerWithSs:
    ...
    def setup(self, scheme, groups, preprocess, mac, Ss_levels = None):
        ...
        if Ss_levels is not None:
            self.Ss_levels = Ss_levels
            return
        Ss_path = os.path.join("best_Ss",self.args.env_args["task_type"]) 
        if not os.path.exists("best_Ss"):
            os.makedirs("best_Ss")
        if os.path.exists(Ss_path):
            with open(Ss_path, 'rb') as file:  
                # 从文件中加载对象  
                self.Ss_levels = pickle.load(file)
        else:
            # env_Ss_partial = env_REGISTRY[self.args.env]
            env_Ss_args = self.args.env_args.copy()
            env_Ss_args['mode'] = "test"
            # env_Ss = env_Ss_partial(**env_Ss_args)
            env_Ss = make_env(env_Ss_args['task_type'],wrapper_names=["OracleWrapper"], mode='test')
            # env_Ss.reset()
            best_S, best_s = search_sS(env_Ss)
            Ss_levels = np.array((best_S, best_s))
            with open("best_Ss/{}".format(env_Ss_args['task_type']), 'wb') as f:
                pickle.dump(Ss_levels, f)
            self.Ss_levels = Ss_levels
            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if idx in envs_not_terminated:  # We produced actions for this env
                    if not terminated[
                        ...
                        parent_conn.send(("step_with_Ss", (cpu_actions[action_idx], self.Ss_levels)))
...
def env_worker(remote, env_fn):
    ...
    while True:
        ...
        if cmd == "step_with_Ss":
            action_from_rl, Ss_levels = data
            Ss_probability = 0.1
            if np.random.rand() < Ss_probability:
                mean_demand = env._env.get_demand_mean()
                replenish = (env._env.get_in_stock() + env._env.get_in_transit()) / (mean_demand + 0.0001)
                replenish = np.where(replenish < Ss_levels[1], Ss_levels[0] - replenish, 0)
                ...
                actions = action_from_rl.flatten()
            # Take a step in the environment
            reward, terminated, env_info = env.step(actions.flatten())
