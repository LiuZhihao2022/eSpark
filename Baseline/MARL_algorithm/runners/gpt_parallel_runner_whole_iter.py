...
class GptParallelRunnerWholeIter:
    ...
    def run(self, lbda_index=None, test_mode=False,
            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if idx in envs_not_terminated:  # We produced actions for this env
                    if not terminated[
                        ...
                        parent_conn.send(("step", (cpu_actions[action_idx], self.mask_func)))
        ...
        if test_mode:
            ...
            return self.batch, cur_stats, profit, total_mask_distribution, mask_components_distribution, cpu_action_episode
    ...
    def run_iter(self, shared_expected_duration_dict, shared_completed_task, shared_successes_task):
        ...
        while ep < self.args.iter_nepisode:
            # 首先查看共享状态。如果在这里检测到共享状态中标志任务已经结束，说明主进程中结束了这个进程。那么就kill掉任务并且return
            if shared_completed_task[self.subthread_id] == True:
                self.logger.console_logger.info(f"thread {self.subthread_id} terminate, stop...")
                self.close_env()
                return (False, "expected duration exceeds max allowed time!")
            ...
            if len(result) == 2 and result[0] == False:
                ...
                batch, cur_stats, profit, total_mask_distribution, mask_components_distribution, cpu_action_episode = result
            ...
            if ep % 100 == 0:
                ...
                action_chosen_times = np.zeros((self.env_info["n_warehouses"], self.env_info["n_actions"]))
                for j in range(self.env_info['n_warehouses']):
                    warehouse_agent_actions = cpu_action_list[:,
                                              j * agents_per_warehouse:(j + 1) * agents_per_warehouse].reshape(-1)
                    max_action = warehouse_agent_actions.max()
                    action_counts = np.bincount(warehouse_agent_actions, minlength=max_action + 1)
                    action_chosen_times[j, :len(action_counts)] += action_counts
            ...
            if self.args.iter_nepisode * self.current_iter + ep - self.last_evaluate_ep >= self.args.evaluate_nepisode and (
                ...
                if ep != 0:
                    ...
                    # subthread_id的预估总共完成时间。若超过1.5倍则需要终止
                    shared_expected_duration_dict[self.subthread_id] = max(self.args.iter_nepisode - ep,
                                                                           0) / ep * time_elapsed + time_elapsed

        shared_expected_duration_dict[self.subthread_id] = time.time() - begin_time
        shared_completed_task[self.subthread_id] = True
        ...
        # 终于到return了，将成功置为true
        shared_successes_task[self.subthread_id] = True
        ...
        if cmd == "step":
            try:
                ...
                if mask_func is not None and not np.any(terminated):
                    total_mask, mask_components = mask_func(env._env.agent_states, env._env.supply_chain,
                                                            env._env.config['action']['space'])
