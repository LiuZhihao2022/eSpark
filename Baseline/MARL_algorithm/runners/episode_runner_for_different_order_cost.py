...
class EpisodeRunner:
    ...
    def run(self, test_mode=False):
        ...
        while not terminated:
            ...
            # replenish = np.ones_like(actions)*50
            # replenish = np.where(replenish >= 0, replenish, 0) / (self.env.get_demand_mean() + 0.00001)
            # actions[-1] = np.ones_like(actions[-1])*10
            # TODO: directly set the replenishment equals to the demand yesterday
            actions[0][0,-1] = 10
