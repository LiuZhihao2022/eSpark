            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if idx in envs_not_terminated:  # We produced actions for this env
                    if not terminated[
                        ...
                        parent_conn.send(("step_top_layer", cpu_actions[action_idx]))
...
def env_worker(remote, env_fn):
    ...
    while True:
        ...
        if cmd == "step":
            ...
            lower_actions= np.array([[np.argmin(np.abs(np.array(discrete_action) - a)) for a in row]  
                for row in replenish])
            # RL algorithm with BSs. Top layer uses RL algorithm and others replenish 50.
            actions[-1] = lower_actions[-1]
