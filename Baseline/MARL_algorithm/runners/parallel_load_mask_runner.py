...
class ParallelLoadMaskRunner:
    def __init__(self, args, logger, mask_func):
        self.args = args
        self.logger = logger
        ...
        self.ps = [
            Process(
                target=env_worker,
                args=(worker_conn, CloudpickleWrapper(partial(env_fn, **env_arg)), mask_func),
            )
...
def env_worker(remote, env_fn, mask_func):
    # Make environment
    env = env_fn.x()
    while True:
