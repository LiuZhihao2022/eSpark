...
def time_left_gpt(start_time, gpt_update_iter, ep, iter_nepisode, iters):
    total_ep = iters*iter_nepisode
    current_ep = ep + gpt_update_iter*iter_nepisode
    time_elapsed = time.time() - start_time
    time_left = time_elapsed * (total_ep - current_ep) / current_ep
    # Just in case its over 100 days
    time_left = min(time_left, 60 * 60 * 24 * 100)
    return time_str(time_left)
