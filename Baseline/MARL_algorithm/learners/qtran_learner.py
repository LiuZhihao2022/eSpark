...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, current_iter: str = ""):
        ...
        if self.args.mixer == "qtran_base":
            ...
            if not self.args.use_double_q: # Already computed if we're doing double Q-Learning
                max_actions_current_ = th.zeros(size=(batch.batch_size, batch.max_seq_length, self.args.n_agents, self.args.n_actions), device=batch.device )
                max_actions_current_onehot = max_actions_current_.scatter(3, max_actions_current[:, :], 1)
            max_joint_qs, _1 = self.mixer(batch[:, :-1], mac_hidden_states[:,:-1], actions=max_actions_current_onehot[:,:-1]) # Don't use the target network and target agent max actions as per author's email
        ...
        if "gpt" not in self.args.runner:
            if self.args.use_wandb:
                wandb.log({
                    ...
                    "nopt_loss": nopt_loss.item(), 
                    "grad_norm": grad_norm,
                    "td_error_abs": (masked_td_error.abs().sum().item()/mask_elems),
                    "td_targets": ((masked_td_error).sum().item()/mask_elems),
                    "td_chosen_qs": (joint_qs.sum().item()/mask_elems),
                    "v_mean": (vs.sum().item()/mask_elems),
                    "agent_indiv_qs": ((chosen_action_qvals * mask).sum().item()/(mask_elems * self.args.n_agents))

                })
        else:
            self.training_curve_log.append({
                "loss": loss.item(),
                "td_loss": td_loss.item(),
                "opt_loss": opt_loss.item(),
                "nopt_loss": nopt_loss.item(), 
    ...
    def cuda(self,gpu_id=0):
        ...
        if self.mixer is not None:
            self.mixer.cuda(gpu_id)
            self.target_mixer.cuda(gpu_id)
    ...
    def load_models(self, path):
        postfix = os.listdir(path)[0].split('_')[-1][:-3]
    ...
    def load_other_learner(self, other_learner):
        ...
        self.last_target_update_episode = other_learner.last_target_update_episode

        self.mixer.load_state_dict(other_learner.mixer.state_dict())

        self.params += list(self.mixer.parameters())
        self.target_mixer.load_state_dict(other_learner.target_mixer.state_dict())

        self.optimiser.load_state_dict(other_learner.optimiser.state_dict())

        self.target_mac.load_state(other_learner.mac)
