...
class LocalQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.args.use_n_lambda:
            ...
            actions = actions.unsqueeze(dim=-2).expand(
                actions.shape[0], actions.shape[1], actions.shape[2], self.args.n_lambda, actions.shape[-1])
        ...
        with torch.no_grad():
            ...
            if self.args.use_n_lambda:
                target_mac_out = target_mac_out.reshape(
                    target_mac_out.shape[0], target_mac_out.shape[1], target_mac_out.shape[2], self.args.n_lambda, -1)
                avail_actions = avail_actions.unsqueeze(dim=-2).expand(
                    avail_actions.shape[0], avail_actions.shape[1], avail_actions.shape[2], self.args.n_lambda, avail_actions.shape[-1])
            ...
            if self.args.use_n_lambda:
                terminated = terminated.unsqueeze(dim=-2)
        ...
        wandb_dict = {}
        if self.args.use_n_lambda:
            if self.args.use_loss_normalization:
                td_error = td_error / torch.std(td_error, dim=(0, 1, 2), keepdim=True)
            
            for lbda_index in [0, 5, 15, 20, 30, 40, 50]:
                wandb_dict.update({'train_q_{:02d}_loss'.format(lbda_index // 5): \
                    float(td_error[:,:,:,lbda_index].mean().detach())})

            if self.args.use_sample_prob_weights:
                epsilon = 0.4
                weights = \
                    (mac_out[:, :-1].argmax(dim=-1) == actions.squeeze(-1)) \
                    * (1 - epsilon) \
                    + (mac_out[:, :-1].argmax(dim=-1) != actions.squeeze(-1)) \
                    * epsilon / (1 - self.n_actions)
                weights = (mac_out[:, :-1].argmax(dim=-1) == actions.squeeze(-1)) * (1 - epsilon) + (mac_out[:, :-1].argmax(dim=-1) != actions.squeeze(-1)) * epsilon / (self.n_actions - 1)
            else:
                weights = 1

            q_loss = (weights * td_error).mean()
